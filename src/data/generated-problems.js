...
// Generated at: 2025-09-09T17:41:57.814Z
...
const problemBank = [
  ...
  {
    "id": 20,
    "title": "Function Pointers - Advanced Callback and Dispatch Systems",
    "difficulty": 8,
    "xp": 70,
    "phase": "foundations",
    ...
    "skills": [
      ...
      "Implementing callback registration",
      "Building dispatch tables",
      "Calling functions through pointers",
      "Managing handler registration",
      "Professional kernel callback patterns"
    ],
    "files": [
      {
        "name": "function_pointers.h",
        "content": "#ifndef FUNCTION_POINTERS_H\n#define FUNCTION_POINTERS_H\n\n#include <linux/module.h>\n#include <linux/kernel.h>\n#include <linux/init.h>\n#include <linux/types.h>\n#include <linux/slab.h>\n\n#define MAX_HANDLERS 5\n#define MAX_NAME_LEN 32\n\n/* Record type constants */\n#define SENSOR_RECORD 1\n#define DEVICE_RECORD 2\n#define STATUS_RECORD 3\n\n/* Function pointer typedef declarations (like STDF handler functions) */\ntypedef int (*record_processor_t)(void *record_data, uint32_t record_id);\ntypedef void (*error_handler_t)(const char *error_message);\ntypedef void (*status_callback_t)(uint8_t record_type, const char *status);\n\n/* Dispatch table entry (like STDF record dispatch) */\ntypedef struct {\n    uint8_t record_type;\n    const char *type_name;\n    record_processor_t processor;\n    error_handler_t error_handler;\n    status_callback_t status_callback;\n} dispatch_entry;\n\n/* Global dispatch table and handler arrays */\nextern dispatch_entry handler_table[MAX_HANDLERS];\nextern int registered_handlers;\nextern record_processor_t active_processor;\nextern error_handler_t active_error_handler;\nextern status_callback_t active_status_callback;\n\n/* TODO: Declare your functions */\n/* int register_record_handler(uint8_t record_type, record_processor_t processor, error_handler_t error_handler); */\n/* int process_record_with_dispatch(uint8_t record_type, void *record_data, uint32_t record_id); */\n/* void call_status_callback(uint8_t record_type, const char *status_message); */\n/* void display_handler_table(void); */\n\n/* Predefined handler functions for students to use */\nint sensor_record_processor(void *record_data, uint32_t record_id);\nvoid sensor_error_handler(const char *error_message);\nvoid sensor_status_callback(uint8_t record_type, const char *status);\nint device_record_processor(void *record_data, uint32_t record_id);\nvoid device_error_handler(const char *error_message);\nvoid device_status_callback(uint8_t record_type, const char *status);\n\n#endif /* FUNCTION_POINTERS_H */",
        "readOnly": false,
        "language": "h"
      },
      {
        "name": "function_pointers.c",
        "content": "#include \"function_pointers.h\"\n\n/* Global dispatch table and handler tracking */\ndispatch_entry handler_table[MAX_HANDLERS] = {{0}};\nint registered_handlers = 0;\nrecord_processor_t active_processor = NULL;\nerror_handler_t active_error_handler = NULL;\nstatus_callback_t active_status_callback = NULL;\n\nint test_record_type = SENSOR_RECORD;\n\n/* IMPORTANT: DO NOT REMOVE OR MODIFY THE LINES BELOW!\n * These module parameter declarations are required for validation testing.\n * The testing system uses these to verify your function pointer logic works\n * with different input values. Removing them will cause test failures.\n */\nmodule_param(test_record_type, int, 0644);\nMODULE_PARM_DESC(test_record_type, \"Record type for testing function pointer dispatch\");\n/* END OF REQUIRED MODULE PARAMETER SECTION */\n\n/* Predefined handler functions (provided for students) */\nint sensor_record_processor(void *record_data, uint32_t record_id) {\n    printk(KERN_INFO \"Processing sensor record ID: %u\\n\", record_id);\n    printk(KERN_INFO \"Sensor data processed successfully\\n\");\n    return 0;\n}\n\nvoid sensor_error_handler(const char *error_message) {\n    printk(KERN_ERR \"Sensor Error: %s\\n\", error_message);\n}\n\nvoid sensor_status_callback(uint8_t record_type, const char *status) {\n    printk(KERN_INFO \"Sensor Status [Type %u]: %s\\n\", record_type, status);\n}\n\nint device_record_processor(void *record_data, uint32_t record_id) {\n    printk(KERN_INFO \"Processing device record ID: %u\\n\", record_id);\n    printk(KERN_INFO \"Device data processed successfully\\n\");\n    return 0;\n}\n\nvoid device_error_handler(const char *error_message) {\n    printk(KERN_ERR \"Device Error: %s\\n\", error_message);\n}\n\nvoid device_status_callback(uint8_t record_type, const char *status) {\n    printk(KERN_INFO \"Device Status [Type %u]: %s\\n\", record_type, status);\n}\n\n/* PATTERN SCAFFOLDING: How function pointer dispatch works in C */\n/* Basic pattern for function pointer dispatch:\n *\n * typedef int (*function_ptr_t)(void *data);\n * function_ptr_t my_function = some_actual_function;\n * int result = my_function(data);  // Call through function pointer\n *\n * This is like: function_name → function_ptr_t → call through pointer\n */\n\n/* FUNCTION POINTER OPERATORS: */\n/* typedef return_type (*ptr_name)(parameters) defines a function pointer type */\n/* handler_table[i].processor(data, id) calls function through pointer */\n\n/* TODO: Implement handler registration function */\n/* Use the pattern above: register function → store in table → call through pointer */\nint register_record_handler(uint8_t record_type, record_processor_t processor, error_handler_t error_handler) {\n    /* TODO: Check if handler table is full */\n    /* if (registered_handlers >= MAX_HANDLERS) { */\n    /*     printk(KERN_ERR \"Handler table full, cannot register more handlers\\n\"); */\n    /*     return -ENOMEM; */\n    /* } */\n    \n    /* TODO: Find existing handler or create new entry */\n    /* int slot = -1; */\n    /* for (int i = 0; i < registered_handlers; i++) { */\n    /*     if (handler_table[i].record_type == record_type) { */\n    /*         slot = i; */\n    /*         break; */\n    /*     } */\n    /* } */\n    \n    /* TODO: If no existing handler, use next available slot */\n    /* if (slot == -1) { */\n    /*     slot = registered_handlers; */\n    /*     registered_handlers++; */\n    /* } */\n    \n    /* TODO: Store function pointers in dispatch table */\n    /* handler_table[slot].record_type = record_type; */\n    /* handler_table[slot].processor = processor; */\n    /* handler_table[slot].error_handler = error_handler; */\n    \n    /* TODO: Set type name based on record type */\n    /* if (record_type == SENSOR_RECORD) { */\n    /*     handler_table[slot].type_name = \"SENSOR\"; */\n    /*     handler_table[slot].status_callback = sensor_status_callback; */\n    /* } else if (record_type == DEVICE_RECORD) { */\n    /*     handler_table[slot].type_name = \"DEVICE\"; */\n    /*     handler_table[slot].status_callback = device_status_callback; */\n    /* } else { */\n    /*     handler_table[slot].type_name = \"UNKNOWN\"; */\n    /*     handler_table[slot].status_callback = NULL; */\n    /* } */\n    \n    /* TODO: Print registration success message */\n    /* printk(KERN_INFO \"Registered %s record handler\\n\", handler_table[slot].type_name); */\n    \n    return 0;\n}\n\n/* TODO: Implement dispatch function using function pointers */\n/* HINT: Look up handler in table and call through function pointer */\nint process_record_with_dispatch(uint8_t record_type, void *record_data, uint32_t record_id) {\n    /* TODO: Search for handler in dispatch table */\n    /* for (int i = 0; i < registered_handlers; i++) { */\n    /*     if (handler_table[i].record_type == record_type) { */\n    /*         printk(KERN_INFO \"Found handler for record type: %u\\n\", record_type); */\n    \n            /* TODO: Set active function pointers */\n    /*         active_processor = handler_table[i].processor; */\n    /*         active_error_handler = handler_table[i].error_handler; */\n    /*         active_status_callback = handler_table[i].status_callback; */\n    \n            /* TODO: Call processor function through pointer */\n    /*         int result = handler_table[i].processor(record_data, record_id); */\n    \n            /* TODO: Call status callback if available */\n    /*         if (handler_table[i].status_callback) { */\n    /*             handler_table[i].status_callback(record_type, \"processing_complete\"); */\n    /*         } */\n    \n    /*         return result; */\n    /*     } */\n    /* } */\n    \n    /* TODO: Handle case where no handler is found */\n    /* printk(KERN_ERR \"No handler registered for record type: %u\\n\", record_type); */\n    /* return -ENOENT; */\n}\n\n/* TODO: Implement status callback function */\nvoid call_status_callback(uint8_t record_type, const char *status_message) {\n    /* TODO: Find and call status callback for record type */\n    /* for (int i = 0; i < registered_handlers; i++) { */\n    /*     if (handler_table[i].record_type == record_type) { */\n    /*         if (handler_table[i].status_callback) { */\n    /*             handler_table[i].status_callback(record_type, status_message); */\n    /*         } else { */\n    /*             printk(KERN_INFO \"No status callback for record type: %u\\n\", record_type); */\n    /*         } */\n    /*         return; */\n    /*     } */\n    /* } */\n    /* printk(KERN_ERR \"No handler found for status callback, record type: %u\\n\", record_type); */\n}\n\n/* TODO: Implement handler table display function */\nvoid display_handler_table(void) {\n    printk(KERN_INFO \"Handler Table Status:\\n\");\n    printk(KERN_INFO \"Registered handlers: %d/%d\\n\", registered_handlers, MAX_HANDLERS);\n    \n    /* TODO: Display all registered handlers */\n    /* for (int i = 0; i < registered_handlers; i++) { */\n    /*     printk(KERN_INFO \"Slot %d: Type=%u, Name=%s, Processor=%p, ErrorHandler=%p\\n\", */\n    /*            i, handler_table[i].record_type, handler_table[i].type_name, */\n    /*            handler_table[i].processor, handler_table[i].error_handler); */\n    /* } */\n}\n\nstatic int __init function_pointers_init(void)\n{\n    int ret;\n    void *dummy_data = NULL;\n    \n    printk(KERN_INFO \"Function pointers module loaded\\n\");\n    \n    /* Initialize handler table */\n    for (int i = 0; i < MAX_HANDLERS; i++) {\n        handler_table[i].record_type = 0;\n        handler_table[i].type_name = NULL;\n        handler_table[i].processor = NULL;\n        handler_table[i].error_handler = NULL;\n        handler_table[i].status_callback = NULL;\n    }\n    \n    /* Register predefined handlers */\n    ret = register_record_handler(SENSOR_RECORD, sensor_record_processor, sensor_error_handler);\n    if (ret) {\n        printk(KERN_ERR \"Failed to register sensor handler\\n\");\n        return ret;\n    }\n    \n    ret = register_record_handler(DEVICE_RECORD, device_record_processor, device_error_handler);\n    if (ret) {\n        printk(KERN_ERR \"Failed to register device handler\\n\");\n        return ret;\n    }\n    \n    /* Display handler table */\n    display_handler_table();\n    \n    /* Test dispatch with module parameter */\n    printk(KERN_INFO \"Testing dispatch with record type: %d\\n\", test_record_type);\n    ret = process_record_with_dispatch(test_record_type, dummy_data, 1001);\n    if (ret) {\n        printk(KERN_ERR \"Dispatch failed with error: %d\\n\", ret);\n        return ret;\n    }\n    \n    /* Test status callback */\n    call_status_callback(test_record_type, \"module_init_complete\");\n    \n    return 0;\n}\n\nstatic void __exit function_pointers_exit(void)\n{\n    printk(KERN_INFO \"Function pointers module unloading\\n\");\n    \n    /* Reset global function pointers */\n    active_processor = NULL;\n    active_error_handler = NULL;\n    active_status_callback = NULL;\n    registered_handlers = 0;\n    \n    printk(KERN_INFO \"Function pointers module unloaded\\n\");\n}\n\nmodule_init(function_pointers_init);\nmodule_exit(function_pointers_exit);\n\nMODULE_LICENSE(\"GPL\");\nMODULE_AUTHOR(\"Student\");\nMODULE_DESCRIPTION(\"Learning function pointers, callbacks, and dispatch tables\");\nMODULE_VERSION(\"1.0\");",
        "readOnly": false,
        "language": "c"
      },
      {
    ...
    "validation": {
      "exactRequirements": {
        ...
        "function_declarations": [
          ...
          {
            "name": "process_record_with_dispatch",
            "returnType": "int",
            "parameters": [
              "uint8_t record_type",
              "void *record_data",
        ...
        "variables_declarations": [
          ...
          },
          {
            ...
            "type": "typedef",
            "value": "void (*)(const char *error_message)",
            "storageClass": "none"
          },
          {
            "name": "status_callback_t",
            "type": "typedef",
            "value": "void (*)(uint8_t record_type, const char *status)",
            "storageClass": "none"
          },
          {
            "name": "dispatch_entry",
            "type": "typedef struct",
            "value": "{uint8_t record_type; const char *type_name; record_processor_t processor; error_handler_t error_handler; status_callback_t status_callback;}",
            "storageClass": "none"
          },
          {
            "name": "handler_table",
    ...
    "inputOutput": {
      ...
      "requirements": [
        "Use required function names: function_pointers_init, function_pointers_exit, register_record_handler, process_record_with_dispatch, call_status_callback, display_handler_table",
        "Use required variable names: handler_table, registered_handlers, active_processor, active_error_handler, active_status_callback",
        "Print exact message format for backend validation compatibility",
        "Must include: linux/module.h, linux/kernel.h, linux/init.h, linux/types.h, linux/slab.h",
        "Code must contain: handler_table[slot].record_type = record_type, handler_table[slot].processor = processor, handler_table[slot].error_handler = error_handler, active_processor = handler_table[i].processor, active_error_handler = handler_table[i].error_handler, active_status_callback = handler_table[i].status_callback, handler_table[i].processor(record_data, record_id), handler_table[i].status_callback(record_type, status_message), if (registered_handlers >= MAX_HANDLERS), module_param(test_record_type, int, 0644)",
        "Must include MODULE_LICENSE(\"GPL\")"
      ],
      "functionLinkedOutputs": [
        {
