...
// Generated at: 2025-07-12T17:20:37.180Z
...
const problemBank = [
  ...
  },
  {
    ...
    "inputOutput": {
      "expectedOutput": [],
      "requirements": [
        "Test Environment: Implement file_operations (open, read, write, release) and register character device with name 'mychardev' to pass userspace tests.",
        "QEMU Setup: QEMU provides hardware emulation environment with PCI devices",
        "QEMU Setup: Use standard kernel APIs: alloc_chrdev_region(), cdev_init(), cdev_add()",
        "Userspace Test: Test writes string 'Hello from userspace!' (21 bytes) and reads it back",
        "Userspace Test: Must handle lseek(fd, 0, SEEK_SET) to reset file position",
        "Userspace Test: Return exact byte counts: write() and read() must return same number of bytes",
        "Userspace Test: Support copy_to_user() and copy_from_user() for data transfer",
        "Test Setup: Device registration: use alloc_chrdev_region(&device_number, 0, 1, 'mychardev')",
        "Test Setup: File operations: implement .open, .release, .read, .write in struct file_operations",
        "Test Setup: Buffer management: maintain internal buffer to store/retrieve data between write/read calls",
        "Test Setup: Kernel messages: print 'mychardev: driver loaded' and 'mychardev: device.*registered' for test validation"
      ]
    }
  },
  {
